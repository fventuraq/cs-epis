...
function RecursoCrud($http, $q,$rootScope,$location,$route, urlBase) {
	...
	this.validarToken= function(){
		...
		if(!cabecera){
			localStorage.clear();
            location.replace( "/iamd/" );
		   return;
		}
            
		$http({
			 method: 'GET',
			 url: "/iamd/servidor/ServicioLogin.php",
			 headers: cabecera
		}).success( function(){
			 console.log("BIENVENIDO iamd");
...
miApp.constant("urlBase", "/iamd/servidor/servicios/");
...
miApp.service('upload', ["$http", "$q","$rootScope", function ($http, $q, $rootScope){
	this.enviarArchivo = function(imagen,dir){
		...
		return $http.post("/iamd/subirImagen.php", formData, {
